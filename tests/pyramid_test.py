...
@fb_messenger(app_id=12345678, link='https://fbrunner.com', redirect_uri='https://fbrunner.com')
...
@fb_publication(app_id=12345678, link='https://fbrunner.com', redirect_uri='https://fbrunner.com')
def example_fb_publication(request, **kwargs):
    return kwargs['fb_url']


@fb_publication(link='https://fbrunner.com', redirect_uri='https://fbrunner.com')
...
def test_fb_publication_basic(mocker):
...
def test_fb_publication_with_one_parameter_with_post(mocker):
    request = Request(post={"fb_redirect_uri": "http://road.com"})
    fb_pub = example_fb_publication(request=request)
    assert fb_pub == 'https://www.facebook.com/dialog/feed?app_id=12345678&link=https%3A%2F%2Ffbrunner.com&redirect_uri=https%3A%2F%2Ffbrunner.com'


def test_fb_publication_raises(mocker):
    request = Request(post={"fb_link": "http://road.com", "fb_redirect_uri": "http://road.com"})
    with pytest.raises(FbRoadRunnerFieldError):
        example_fb_publication_without_app_id(request=request)


def test_fb_publication_with_env(monkeypatch):
...
def test_example_fb_publication_with_all_parameters_with_post():
    request = Request(
        post={"fb_redirect_uri": "http://road.com", "display": "popup&amp;caption=An%20example%20caption",
              "to": "1234", "from": "7575", "source": "12345", "link": "https://roadrunner.com"})
...
def test_fb_messenger_basic(mocker):
    request = Request(post={})
...
def test_fb_messenger_with_one_parameter_with_post(mocker):
    request = Request(post={"fb_redirect_uri": "http://road.com"})
    fb_message = example_fb_messenger(request=request)
    assert fb_message == 'http://www.facebook.com/dialog/send?app_id=12345678&redirect_uri=https%3A%2F%2Ffbrunner.com&link=https%3A%2F%2Ffbrunner.com'


def test_fb_messenger_raises(mocker):
    request = Request(post={"fb_link": "http://road.com", "fb_redirect_uri": "http://road.com"})
    with pytest.raises(FbRoadRunnerFieldError):
        example_fb_messenger_without_app_id(request=request)


def test_fb_messenger_with_env(monkeypatch):
    monkeypatch.setenv('FB_APP_ID', 123456789)
    request = Request(post={})
...
def test_example_fb_messenger_with_all_parameters_with_post():
    request = Request(
        post={"fb_redirect_uri": "http://road.com", "display": "popup&amp;caption=An%20example%20caption",
